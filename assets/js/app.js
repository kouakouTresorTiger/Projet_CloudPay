...
/* ══════════════════════════════════════════════
   CloudPay — app.js
   • Accès direct à l'app sans connexion (mode visiteur)
   • Auth optionnelle via topbar (Se connecter / Créer un compte)
...
const DB = {
  ...
  findUser(e)  { return this.getUsers().find(u=>u.email.toLowerCase()===e.toLowerCase()); },
  ...
  addTx(t)     { t.id='TXN-'+Date.now(); t.ts=new Date().toISOString(); this.setTx([t,...this.getTx()]); return t; },

  getNotifs()  { return this.get('notifs_'+this._uid()) || []; },
  setNotifs(v) { this.set('notifs_'+this._uid(), v); },
  addNotif(n)  {
    n.id='NTF-'+Date.now(); n.ts=new Date().toISOString(); n.read=false;
    this.setNotifs([n,...this.getNotifs()]);
    updateNotifBadge(); return n;
  },
  markRead()   { this.setNotifs(this.getNotifs().map(n=>({...n,read:true}))); updateNotifBadge(); },

  getEscrows() { return this.get('esc_'  +this._uid()) || []; },
  setEscrows(v){ this.set('esc_'  +this._uid(), v); },
  addEscrow(e) {
    e.id='ESC-'+Date.now(); e.createdAt=new Date().toISOString();
    this.setEscrows([e,...this.getEscrows()]); return e;
  },
  ...
  addApiKey(k) { k.id='KEY-'+Date.now(); k.createdAt=new Date().toISOString(); this.setApiKeys([...this.getApiKeys(),k]); return k; },
...
function defaultDevCfg() {
  return {
    ...
    fraud_rules:{velocity_check:true,location_check:true,card_multi:true,temp_email:true,
                 bot_detection:true,chargeback_block:true,otp_above:50000,max_score:70,action:'block'},
    webhooks:[],
...
/* ─────────────────────────────────────
   ÉTAT GLOBAL
───────────────────────────────────── */
let currentUser    = null;   // null = visiteur
...
/* Compte démo pré-créé */
const DEMO = {
  id:'demo-001', firstName:'Demo', lastName:'Utilisateur',
  email:'demo@cloudpay.ci', phone:'+225 07 00 00 00 00',
  password:'demo1234', type:'developer', balance:245000, pin:'123456',
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!users.find(u=>u.email===DEMO.email)) DB.setUsers([...users, DEMO]);
  ...
  /* date min du séquestre */
  const dl = document.getElementById('escDeadline');
  if (dl) { const d=new Date(); d.setDate(d.getDate()+1); dl.min=d.toISOString().split('T')[0]; }
...
function updateAuthUI() {
  ...
  if (loggedIn) {
    ...
    document.getElementById('sideUserPlan').textContent = {personal:'Particulier',merchant:'Marchand',developer:'Développeur'}[currentUser.type]||'Compte';
...
function doLogin() {
  ...
  if (!email || !pass) { toast('⚠️','Champs requis','Remplissez votre email et mot de passe','warn'); return; }

  const user = DB.findUser(email);
  if (!user)              { toast('❌','Compte introuvable','Aucun compte avec cet email','error'); return; }
  if (user.password!==pass){ toast('❌','Mot de passe incorrect','Vérifiez votre mot de passe','error'); return; }
...
function doRegister() {
  ...
  if (!email.includes('@')){ toast('⚠️','Email invalide','Entrez un email valide','warn'); return; }
  if (pass.length<6){ toast('⚠️','Mot de passe trop court','Minimum 6 caractères','warn'); return; }
  if (DB.findUser(email)){ toast('❌','Email déjà utilisé','Un compte existe avec cet email','error'); return; }

  const newUser = {
    id:'usr-'+Date.now(), firstName:first, lastName:last,
    email, phone, password:pass, type, balance:0, pin:'123456',
  ...
  DB.addNotif({icon:'🎉',title:'Bienvenue sur CloudPay !',body:'Votre compte a été créé avec succès. Explorez les fonctionnalités.',type:'info'});
  nav('dashboard');
}
...
function confirmLogout() {
  showDetailModal('🚪 Déconnexion', `
    <p style="font-size:14px;color:var(--t2);margin-bottom:20px;line-height:1.7;">
      Voulez-vous vous déconnecter ?<br>Vos données sont sauvegardées — reconnectez-vous quand vous voulez.
    </p>
    <div class="g2 gap-8">
...
/* Guard : certaines actions nécessitent un compte */
...
const PAGE_LABELS = {
  dashboard:'Tableau de bord', escrow:'Paiement Sécurisé', recharge:'Recharger',
  transfer:'Transfert', transactions:'Transactions', notifications:'Notifications',
  profile:'Profil', security:'Sécurité', split:'Partage de revenus',
  analytics:'Analytiques', 'merchant-cfg':'Configuration Marchand',
  'api-keys':'Clés API', 'api-docs':'Documentation API',
  'dev-payments':'Configuration Paiements', webhooks:'Webhooks',
  'fraud-cfg':'Anti-Fraude', sdk:'SDK & Plugins',
};
...
function setMode(mode, btn) {
  ...
  buildNav(mode);
  nav(mode==='dev'?'api-keys':mode==='merchant'?'dashboard':'dashboard');
}
...
function buildNav(mode) {
  ...
  navEl.innerHTML = (NAV[mode]||NAV.user).map(item => {
    ...
    const notifHtml = item.notif ? `<span class="nav-badge" id="notifNavBadge" style="display:none;">0</span>` : '';
    const tagHtml   = item.tag   ? `<span class="nav-tag-new">${item.tag}</span>` : '';
    return `<button class="nav-item" id="nav-${item.id}" onclick="nav('${item.id}')">
      <span class="ic">${item.ic}</span><span>${item.txt}</span>${notifHtml}${tagHtml}
    </button>`;
  }).join('');
  updateNotifBadge();
}
...
function afterRender(id) {
  ...
  if (id==='dev-payments')  {} // toggles init inline
...
function pageDashboard() {
  ...
  const blocked  = DB.getEscrows().filter(e=>e.status==='pending'||e.status==='locked').reduce((s,e)=>s+e.amount,0);
...
function escrowCardHTML(e) {
  ...
  const actions = isActive ? `
    ...
    <p style="font-size:11px;color:var(--muted);text-align:center;margin-top:5px;">⚠️ Cliquez uniquement si vous avez reçu et vérifié le colis</p>`
  : `<div style="margin-top:10px;"><span class="chip ${chip}">${lbl}</span></div>`;

  return `
  <div class="escrow-card ${e.status}">
    <div class="ec-header">
      <div><div class="ec-product">${esc(e.productName)}</div><div class="ec-seller">${esc(e.sellerAccount)}</div></div>
    ...
    <div class="ec-row">
      ...
      <span style="color:var(--${isActive&&daysLeft<2?'red':'amber'});font-weight:600;">
        ⏰ ${fmtDate(deadline)} ${isActive?`(${daysLeft}j)`:''}</span>
    </div>
...
function pageEscrow() {
  ...
  const cards = all.length
    ...
    : `<div style="text-align:center;padding:40px;color:var(--muted);">
        ...
        <button class="btn btn-cta btn-sm" style="margin-top:14px;" onclick="${currentUser?"openModal('escrowModal')":"openModal('loginModal')"}">${currentUser?'+ Nouveau paiement':'Connectez-vous pour commencer'}</button>
       </div>`;
  return `
  <div class="page-header">
  ...
  <div class="g2">
    ...
    <div class="card">
      ...
      <div class="step-row" style="margin-bottom:0;"><div class="step-num">5</div><div><div class="step-title">Pas de livraison → Remboursement auto</div><div class="step-desc">À l'échéance, votre argent vous revient</div></div></div>
    </div>
  </div>`;
}
...
function pageTransactions() {
  return `
  <div class="page-header">
    <div><div class="ph-t">📋 Transactions</div><div class="ph-s">Historique complet · Cliquez pour les détails</div></div>
    <button class="btn btn-ghost btn-sm" onclick="exportCSV()">📥 Exporter CSV</button>
  </div>
  <div class="card"><div id="txTableContainer"><p style="padding:20px;text-align:center;color:var(--muted);">Chargement...</p></div></div>`;
}
...
function renderTxTable() {
  ...
  if (!el) return;
  if (!txs.length) {
    el.innerHTML=`<div style="text-align:center;padding:40px;color:var(--muted);"><div style="font-size:40px;margin-bottom:10px;">📋</div><p>Aucune transaction</p></div>`;
    return;
  }
...
function pageNotifications() {
  ...
  return `
  <div class="page-header">
    <div><div class="ph-t">🔔 Notifications</div><div class="ph-s">${notifs.length} notification(s) · Cliquez pour les détails</div></div>
  </div>
  ${notifs.map(n=>{
    ...
    return `<div class="card notif-card" onclick="showNotifDetail('${n.id}')" style="margin-bottom:10px;border-left:3px solid ${bc};">
  ...
  <div class="g2">
    ...
    <div>
      ...
      <div class="card" style="border:1.5px solid var(--r-light);">
        ...
        <p style="font-size:13px;color:var(--t2);margin-bottom:14px;">Vos données sont sauvegardées et vous pourrez vous reconnecter à tout moment.</p>
...
function pageSplit() {
  ...
  <div class="g2">
    <div>
      <div class="card" style="margin-bottom:16px;">
        <div class="card-title">👥 Configuration des bénéficiaires</div>
        <!-- VENDEUR -->
        <div class="split-party-card" style="border-left:4px solid var(--green);background:var(--g-light);">
          <div class="split-party-header">
            ...
            <div class="split-party-info">
              ...
              <div class="split-party-desc">Reçoit le paiement principal pour son produit ou service. Versement immédiat après validation.</div>
            </div>
            <span class="chip chip-green">Obligatoire</span>
          </div>
          <div class="form-group" style="margin-top:10px;margin-bottom:0;">
            <label class="form-label">Pourcentage du vendeur</label>
            <div class="input-group suf">
              <input type="number" class="form-input" id="sp-seller" min="0" max="100" value="${cfg.seller}" oninput="recalcSplitConfig()"/>
              <span class="ig-suf">%</span>
            </div>
            <div class="form-hint">💡 Part reversée au marchand après confirmation de réception du colis par le client</div>
          </div>
        </div>
        <!-- PLATEFORME -->
        <div class="split-party-card" style="border-left:4px solid var(--accent);background:var(--a-light);">
          <div class="split-party-header">
            ...
            <div class="split-party-info">
              ...
              <div class="split-party-desc">Commission de service couvrant la protection escrow, l'anti-fraude IA, et le traitement des paiements mobiles.</div>
            </div>
            <span class="chip chip-indigo">Obligatoire</span>
          </div>
          <div class="form-group" style="margin-top:10px;margin-bottom:0;">
            <label class="form-label">Commission de la plateforme</label>
            <div class="input-group suf">
              <input type="number" class="form-input" id="sp-platform" min="0" max="100" value="${cfg.platform}" oninput="recalcSplitConfig()"/>
              <span class="ig-suf">%</span>
            </div>
            <div class="form-hint">💡 Inclut les frais de traitement Orange Money, MTN, Wave, Mastercard, PayPal...</div>
          </div>
        </div>
        <!-- LIVREUR (facultatif) -->
        <div class="split-party-card" id="deliveryCard" style="border-left:4px solid ${cfg.delivery_enabled?'var(--amber)':'var(--border)'};background:${cfg.delivery_enabled?'var(--am-light)':'var(--s2)'};">
          <div class="split-party-header">
            ...
            <div class="split-party-info">
              ...
              <div class="split-party-desc">Frais de livraison versés directement au livreur partenaire lors de la validation. Activez uniquement si vous travaillez avec un livreur externe.</div>
            </div>
          ...
          <div id="deliveryFields" style="display:${cfg.delivery_enabled?'block':'none'};margin-top:10px;">
            <div class="form-group" style="margin-bottom:0;">
              <label class="form-label">Pourcentage du livreur</label>
              <div class="input-group suf">
                <input type="number" class="form-input" id="sp-delivery" min="0" max="100" value="${cfg.delivery}" oninput="recalcSplitConfig()"/>
                <span class="ig-suf">%</span>
              </div>
              <div class="form-hint">💡 Ce montant est déduit automatiquement et versé au livreur dès la validation</div>
            </div>
          </div>
        </div>
        <!-- TOTAL -->
        <div class="split-total" id="splitConfigTotal" style="margin-top:12px;">
          <span>Total réparti</span><span id="splitConfigPct">—</span>
        </div>
        <button class="btn btn-cta btn-full" style="margin-top:14px;" onclick="saveSplitConfig()">💾 Enregistrer la configuration</button>
      </div>
    </div>
    <div>
      <div class="card" style="margin-bottom:16px;">
        <div class="card-title">🧮 Simulateur</div>
        <div class="form-group"><label class="form-label">Montant simulé (FCFA)</label>
          <div class="input-group"><span class="ig-pre">FCFA</span><input type="number" class="form-input" id="simAmt" value="100000" oninput="recalcSplitConfig()"/></div>
        </div>
      ...
      <div class="card">
        ...
        ${rules.length?rules.map(r=>`
          ...
          </div>`).join(''):'<p style="text-align:center;color:var(--muted);padding:20px;font-size:13px;">Aucune configuration sauvegardée</p>'}
      </div>
    </div>
  </div>`;
}
...
function pageApiKeys() {
  ...
  <div class="alert alert-warn" style="margin-bottom:16px;">
    ...
    <div class="alert-body">Stockez-la dans une variable d'environnement (.env). Ne la commitez jamais dans Git.</div></div>
  </div>
...
<span class="fn">composer require</span> <span class="str">cloudpay/cloudpay-php</span></pre>
      <button class="copy-btn" onclick="copyText('npm install cloudpay-js')">Copier npm</button>
    </div>
...
function apiKeyRowHTML(k) {
  ...
  <div class="api-key-box" style="margin-bottom:10px;flex-direction:column;gap:8px;">
    <div style="display:flex;align-items:center;gap:8px;">
      ...
      <button class="btn btn-ghost btn-sm" onclick="copyText('${k.pk}')">📋</button>
    </div>
    <div style="display:flex;align-items:center;gap:8px;">
      ...
      <button class="btn btn-ghost btn-sm" onclick="revealSk('${k.id}','${k.sk}')">👁</button>
      <button class="btn btn-ghost btn-sm" onclick="copyText('${k.sk}')">📋</button>
    </div>
    <div style="display:flex;align-items:center;justify-content:space-between;">
      <span style="font-size:11px;color:var(--muted);">${k.name||'Sans nom'} · ${fmtDateShort(new Date(k.createdAt))}</span>
      <button class="btn btn-ghost btn-sm" style="color:var(--red);" onclick="deleteApiKey('${k.id}')">🗑 Supprimer</button>
    </div>
  </div>`;
}

/* ════ API DOCS (schéma + explications clés) ════ */
function pageApiDocs() {
  ...
  <div class="card" style="margin-bottom:20px;">
    ...
    <div style="overflow-x:auto;">
      <div style="min-width:640px;display:flex;align-items:center;gap:0;">
        ...
        ${schemaArrow()}
        ${schemaStep('2','🔑','Clés API','pk_ côté client sk_ côté serveur','#7c3aed','rgba(124,58,237,.1)')}
        ${schemaArrow()}
  ...
  <!-- TYPES DE CLÉS -->
  <div class="card" style="margin-bottom:20px;">
    <div class="card-title">🔑 Les 4 types de clés API — Lesquelles utiliser et quand ?</div>
    <div class="g2" style="gap:12px;">
      <div style="background:var(--g-light);border:1px solid rgba(16,185,129,.2);border-radius:var(--rs);padding:16px;">
        ...
        <ul style="font-size:12px;color:var(--t2);padding-left:16px;line-height:1.9;margin-top:10px;">
          <li><strong>Usage :</strong> Côté client (navigateur, app mobile, React, Vue)</li>
          <li><strong>Sécurité :</strong> Peut être exposée dans le code front-end</li>
          <li><strong>Commence par :</strong> <code>pk_test_</code> ou <code>pk_live_</code></li>
        </ul>
        <div class="form-hint" style="margin-top:8px;">✅ Sûre dans HTML, React, app mobile</div>
      </div>
      ...
      <div style="background:var(--b-light);border:1px solid rgba(59,130,246,.2);border-radius:var(--rs);padding:16px;">
        ...
        <ul style="font-size:12px;color:var(--t2);padding-left:16px;line-height:1.9;margin-top:10px;">
          <li><strong>Usage :</strong> Vérifier les signatures des webhooks reçus</li>
          <li><strong>Comment :</strong> HMAC-SHA256 de chaque payload entrant</li>
          <li><strong>Où :</strong> Côté serveur dans votre handler webhook</li>
        </ul>
      </div>
      <div style="background:var(--am-light);border:1px solid rgba(245,158,11,.2);border-radius:var(--rs);padding:16px;">
        ...
        <ul style="font-size:12px;color:var(--t2);padding-left:16px;line-height:1.9;margin-top:10px;">
          ...
          <li><strong>Permissions :</strong> Configurables (read/write par ressource)</li>
        </ul>
      </div>
    </div>
  </div>
  ...
  <div class="card" style="margin-bottom:20px;">
    ...
    <div class="step-row">
      ...
      <div style="flex:1;">
        <div class="step-title">Installer le SDK</div>
...
<span class="fn">composer require</span> <span class="str">cloudpay/cloudpay-php</span></pre>
          <button class="copy-btn" onclick="copyText('npm install cloudpay-js')">Copier</button>
        </div>
      </div>
    </div>
    <div class="step-row">
      ...
      <div style="flex:1;">
        <div class="step-title">Variables d'environnement</div>
        <div class="code-block" style="margin-top:8px;"><pre><span class="cm"># .env (ne jamais commiter ce fichier !)</span>
<span class="key">CLOUDPAY_SECRET_KEY</span>=<span class="str">sk_live_xxxxxxxxxxxxxxxxxxxx</span>
<span class="key">CLOUDPAY_WEBHOOK_SECRET</span>=<span class="str">whsec_xxxxxxxxxxxxxxxxxxxx</span>
<span class="key">CLOUDPAY_ENV</span>=<span class="str">production</span></pre>
          <button class="copy-btn" onclick="copyText('CLOUDPAY_SECRET_KEY=sk_live_xxx')">Copier</button>
        </div>
      </div>
    </div>
    <div class="step-row">
      ...
      <div style="flex:1;">
        <div class="step-title">Initialiser le client (côté serveur)</div>
...
<span class="kw">const</span> client = <span class="kw">new</span> <span class="fn">CloudPay</span>({
  ...
  <span class="key">environment</span>: process.env.<span class="val">CLOUDPAY_ENV</span>
          ...
          <button class="copy-btn" onclick="copyText('const client = new CloudPay({ apiKey: process.env.CLOUDPAY_SECRET_KEY })')">Copier</button>
        </div>
      </div>
    </div>
    <div class="step-row">
      ...
      <div style="flex:1;">
        <div class="step-title">Créer un paiement sécurisé</div>
  ...
  <span class="key">amount</span>:   <span class="num">125000</span>,         <span class="cm">// FCFA</span>
  <span class="key">currency</span>: <span class="str">'XOF'</span>,
...
<span class="fn">redirect</span>(escrow.<span class="val">checkout_url</span>);</pre>
          <button class="copy-btn" onclick="copyText('const escrow = await client.escrow.create({...})')">Copier</button>
        </div>
      </div>
    </div>
    <div class="step-row">
      ...
      <div style="flex:1;">
        <div class="step-title">Configurer un webhook</div>
...
app.<span class="fn">post</span>(<span class="str">'/webhook/cloudpay'</span>, (req, res) => {
  <span class="kw">const</span> event = client.webhooks.<span class="fn">construct</span>(
    req.body,
    req.headers[<span class="str">'x-cloudpay-signature'</span>],
  ...
  <span class="kw">if</span> (event.type === <span class="str">'escrow.validated'</span>) {
    <span class="fn">onDeliveryConfirmed</span>(event.data);
  }
  res.<span class="fn">json</span>({ received: <span class="kw">true</span> });
});</pre>
          <button class="copy-btn" onclick="copyText('app.post(\"/webhook/cloudpay\", ...)')">Copier</button>
        </div>
      </div>
    </div>
    <div class="step-row" style="margin-bottom:0;">
      ...
      <div style="flex:1;">
        <div class="step-title">Données de test Sandbox</div>
        <div class="code-block" style="margin-top:8px;"><pre><span class="cm">// Numéros de test CloudPay Sandbox</span>
<span class="cm">// Orange Money :</span>   <span class="str">+22500000000</span>  → <span class="val">succès</span>
<span class="cm">// MTN MoMo :       </span><span class="str">+22500000001</span>  → <span class="val">succès</span>
<span class="cm">// Carte Visa :      </span><span class="str">4111 1111 1111 1111</span>  → <span class="val">succès</span>
<span class="cm">// Carte refusée :   </span><span class="str">4000 0000 0000 0002</span>  → <span class="val">refus</span></pre>
          <button class="copy-btn" onclick="copyText('4111 1111 1111 1111')">Copier carte test</button>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="card" style="margin-bottom:16px;">
    ...
    ${epHTML('GET','/escrow/{id}','Consulter un escrow',`{ "id":"esc_xxx","status":"pending","amount":125000,"fraud_score":8,"locked_by_seller":false }`)}
    ${epHTML('POST','/charge','Paiement direct (sans escrow)',`{ "amount":5000,"currency":"XOF","method":"orange_money","phone":"+22507xxx","description":"Abonnement" }`)}
    ${epHTML('GET','/transactions','Lister les transactions',`GET /transactions?status=pending&limit=20&page=1&from=2026-01-01&to=2026-12-31`)}
  </div>

  <!-- CODES HTTP -->
  <div class="card">
    <div class="card-title">📊 Codes de réponse & Événements Webhook</div>
    <div class="g2">
      <div>
        <div class="section-title">Codes HTTP</div>
        ${[['200','var(--green)','OK — Succès'],['201','var(--green)','Created — Ressource créée'],['400','var(--amber)','Bad Request — Paramètre invalide'],['401','var(--amber)','Unauthorized — Clé invalide'],['402','var(--red)','Payment Required — Solde insuffisant'],['403','var(--red)','Forbidden — Accès refusé'],['422','var(--red)','Unprocessable — Fraude détectée'],['429','var(--red)','Too Many Requests — Limite 100/min']]
      ...
      <div>
        <div class="section-title">Événements Webhook</div>
        ${[['escrow.created','Nouveau paiement sécurisé créé'],['escrow.validated','Livraison validée → fonds distribués'],['escrow.refunded','Délai dépassé → remboursement auto'],['escrow.locked','Vendeur a verrouillé l\'annulation'],['payment.completed','Paiement direct réussi'],['fraud.detected','IA a bloqué une transaction'],['split.executed','Distribution automatique terminée'],['transfer.sent','Transfert émis avec succès']]
...
function epHTML(method, path, desc, code, params) {
  ...
  const paramsHtml = params?`<div class="section-title" style="margin-top:10px;">Paramètres</div>
  ...
  return `<div class="endpoint">
    <div class="ep-header" onclick="toggleEp(this)">
      <span class="ep-method ${mc}">${method}</span><span class="ep-path">${path}</span>
      <span class="ep-desc">${desc}</span><span class="ep-arrow">▼</span>
    </div>
    <div class="ep-body">${paramsHtml}
      <div class="section-title" style="margin-top:8px;">Exemple</div>
      <div class="code-block"><pre>${esc(code)}</pre><button class="copy-btn" onclick="copyText('${code.replace(/'/g,"\\'")}')">Copier</button></div>
    </div>
  </div>`;
}

function schemaStep(n,icon,title,desc,color,bg) {
  return `<div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:6px;min-width:80px;">
    <div style="width:48px;height:48px;border-radius:50%;background:${bg};border:2px solid ${color};display:flex;align-items:center;justify-content:center;font-size:20px;position:relative;">
      ${icon}<span style="position:absolute;top:-4px;right:-4px;background:${color};color:#fff;font-size:9px;font-weight:800;width:16px;height:16px;border-radius:50%;display:flex;align-items:center;justify-content:center;">${n}</span>
    </div>
    <div style="text-align:center;"><div style="font-size:11px;font-weight:700;">${title}</div><div style="font-size:10px;color:var(--muted);max-width:80px;line-height:1.3;">${desc}</div></div>
  </div>`;
}
...
function pageDevPayments() {
  ...
  const methods = [
    ...
    {id:'wave',icon:'🌊',name:'Wave CI',color:'#0099ff',desc:'Wave Mobile Money — Afrique de l\'Ouest',
     fields:[{id:'merchantId',lbl:'Business ID Wave',hint:'Disponible dans votre espace Wave for Business'},{id:'apiKey',lbl:'Secret Key Wave',hint:'Utilisée pour signer les requêtes HMAC — clé secrète Wave Business'},{id:'webhook',lbl:'Webhook URL Wave',hint:'Wave enverra les notifications de paiement à cette URL'}]},
    ...
    {id:'mastercard',icon:'💳',name:'Mastercard (MPGS)',color:'#eb001b',desc:'Mastercard Payment Gateway Services',
     fields:[{id:'merchantId',lbl:'Merchant ID MPGS',hint:'ID attribué par votre acquéreur bancaire (ex: SGCI, Ecobank)'},{id:'apiKey',lbl:'API Username MPGS',hint:'Identifiant API du portail Merchant Administration (MA)'},{id:'secretKey',lbl:'API Password / Secret MPGS',hint:'Mot de passe API MPGS — JAMAIS exposé côté client — toujours côté serveur'},{id:'endpoint',lbl:'Endpoint MPGS',hint:'Ex: https://ap.gateway.mastercard.com/api/rest/version/70'}]},
    {id:'visa',icon:'💙',name:'Visa (CyberSource)',color:'#1a1f71',desc:'Visa CyberSource — Paiements carte internationale',
     fields:[{id:'merchantId',lbl:'Merchant ID CyberSource',hint:'ID dans le portail Business Center CyberSource'},{id:'apiKey',lbl:'Shared Secret Key HMAC',hint:'Utilisée pour signer les requêtes HMAC-256 — côté serveur uniquement'},{id:'secretKey',lbl:'API Key ID CyberSource',hint:'Généré dans EBC (Enterprise Business Center) de Visa CyberSource'},{id:'endpoint',lbl:'Endpoint CyberSource',hint:'https://api.cybersource.com (prod) ou https://apitest.cybersource.com (test)'}]},
    {id:'paypal',icon:'🅿️',name:'PayPal REST API',color:'#0070ba',desc:'Paiements internationaux PayPal',
     fields:[{id:'clientId',lbl:'Client ID PayPal',hint:'Créez une app sur developer.paypal.com et copiez le Client ID'},{id:'clientSecret',lbl:'Client Secret PayPal',hint:'Secret de votre application PayPal — JAMAIS dans le code front-end'},{id:'mode',lbl:'Mode (sandbox ou live)',hint:'Mettez "sandbox" pour tester, "live" pour la production — changez en .env'}]},
    {id:'stripe',icon:'🟣',name:'Stripe',color:'#635bff',desc:'Stripe API — Cartes monde entier + SEPA',
     fields:[{id:'publishableKey',lbl:'Publishable Key Stripe (pk_)',hint:'Commence par pk_test_ ou pk_live_ — peut être exposée côté client'},{id:'secretKey',lbl:'Secret Key Stripe (sk_)',hint:'Commence par sk_test_ ou sk_live_ — JAMAIS dans le code client'},{id:'webhookSecret',lbl:'Webhook Signing Secret (whsec_)',hint:'Généré dans le Dashboard Stripe > Webhooks > Signing secret — pour valider les events'}]},
    {id:'crypto',icon:'₿',name:'Crypto (Bitcoin, USDT)',color:'#f7931a',desc:'Paiements crypto via wallet',
     fields:[{id:'walletAddress',lbl:'Adresse Wallet de réception',hint:'Votre adresse publique Bitcoin, Ethereum ou USDT TRC-20 pour recevoir les paiements'},{id:'network',lbl:'Réseau blockchain',hint:'BTC (Bitcoin), ETH (Ethereum), USDT-TRC20 (Tron), USDT-ERC20 (Ethereum), MATIC...'}]},
  ];

  return `
  <div class="page-header">
    <div><div class="ph-t">💳 Configuration Paiements</div><div class="ph-s">Activez et configurez chaque méthode de paiement pour votre intégration</div></div>
  ...
  <div class="alert alert-info" style="margin-bottom:20px;">
    ...
    <div class="alert-body">1. Activez le toggle d'un moyen de paiement · 2. Remplissez les champs API avec les credentials obtenus chez le fournisseur · 3. Configurez votre URL Webhook · 4. Cliquez "Sauvegarder tout" · 5. Testez avec le bouton "Tester la connexion" en mode sandbox</div></div>
  </div>
  <div class="pm-config-grid">
    ${methods.map(m=>{
      const conf = pm[m.id]||{enabled:false};
      return `<div class="pm-config-card ${conf.enabled?'enabled':''}" id="pmcard-${m.id}">
...
function pageWebhooks() {
  return `
  <div class="page-header">
    <div><div class="ph-t">🔗 Webhooks</div><div class="ph-s">Recevez les événements CloudPay en temps réel</div></div>
  ...
  <div class="g2">
    ...
    <div class="card">
      ...
      <div class="alert alert-info" style="margin-top:12px;"><div class="alert-icon">🔐</div><div><div class="alert-title">Vérification signature</div><div class="alert-body">Validez toujours la signature HMAC-SHA256 avant de traiter l'événement pour garantir l'authenticité.</div></div></div>
    </div>
  </div>`;
}
...
function pageFraudCfg() {
  ...
  return `
  <div class="page-header">
    <div><div class="ph-t">🛡️ Anti-Fraude IA</div><div class="ph-s">Score de risque 0-100 calculé pour chaque transaction</div></div>
    <button class="btn btn-cta btn-sm" onclick="saveFraudConfig()">💾 Sauvegarder</button>
  </div>
  <div class="g2">
    <div class="card">
      <div class="card-title">Règles de détection</div>
      <div class="alert alert-info" style="margin-bottom:14px;"><div class="alert-icon">🤖</div><div><div class="alert-title">Détection par IA</div><div class="alert-body">Chaque transaction reçoit un score 0-100. Au-dessus du seuil configuré, l'action définie est appliquée automatiquement.</div></div></div>
    ...
    <div>
      <div class="card" style="margin-bottom:16px;">
        ...
        <div class="form-group"><label class="form-label">Score max accepté (0–100)<div class="form-hint">Au-dessus de ce score, l'action définie ci-dessous s'applique</div></label><div class="input-group suf"><input type="number" class="form-input" id="fr-score" min="0" max="100" value="${fr.max_score||70}"/><span class="ig-suf">/100</span></div></div>
        ...
        <button class="btn btn-cta btn-full" onclick="saveFraudConfig()">Appliquer</button>
      </div>
      <div class="card">
...
function pageSDK() {
  ...
  <div class="g2">
    <div class="card" style="margin-bottom:16px;">
      ...
      ${[['🟨','JavaScript / Node.js','npm install cloudpay-js','ESM & CommonJS · TypeScript inclus · Browser compatible'],['🐍','Python','pip install cloudpay','Compatible Django, Flask, FastAPI, Celery'],['🐘','PHP','composer require cloudpay/cloudpay-php','Laravel, Symfony, WordPress, CodeIgniter'],['🦫','Go','go get github.com/cloudpay/go-cloudpay','Goroutines · context support · idiomatic Go']]
        .map(([ic,lang,cmd,docs])=>`<div style="display:flex;align-items:center;gap:12px;padding:12px 0;border-bottom:1px solid var(--border);">
    ...
    <div>
      <div class="card" style="margin-bottom:16px;">
        ...
        ${[['🛒','WooCommerce Plugin','WordPress · v2.1.0','Stable'],['🟣','Shopify App','Shopify App Store · v1.8','Stable'],['🦋','PrestaShop Module','PrestaShop · v3.0','Stable'],['🔶','Magento Extension','Magento 2 · v1.2','Beta'],['⚛️','React Component','npm · cloudpay-react','Stable'],['💚','Vue.js Plugin','npm · cloudpay-vue','Stable']]
          .map(([ic,name,plat,status])=>`<div style="display:flex;align-items:center;gap:10px;padding:10px 0;border-bottom:1px solid var(--border);">
...
function syncEscrowAmount() {
  const p = document.getElementById('escPrice')?.value;
  const a = document.getElementById('escAmount');
  if (a && !a.value && p) a.value = p;
  updateEscrowSummary();
}
...
function updateEscrowSummary() {
  ...
  if (name && amount && seller && deadline) {
    ...
    sumEl.innerHTML = `
      ...
      <div class="kv-row"><span class="kv-key">Frais (1%)</span><span class="kv-val">${fmtAmt(fee)} FCFA</span></div>
      <div class="kv-row" style="border-bottom:none;"><span class="kv-key" style="font-weight:700;">Total débité</span><span class="kv-val" style="color:var(--accent);font-size:15px;">${fmtAmt(amount+fee)} FCFA</span></div>`;
  } else {
...
function submitEscrow() {
  ...
  if (amount+fee > currentUser.balance) {
    toast('❌','Solde insuffisant',`Solde: ${fmtAmt(currentUser.balance)} FCFA · Requis: ${fmtAmt(amount+fee)} FCFA`,'error'); return;
  }
...
function confirmDelivery() {
  ...
  DB.addTx({type:'debit',amount:-pendingValidation.amount,description:`Libéré · ${pendingValidation.productName}`,status:'done',statusLabel:'Livré ✅'});
  DB.addNotif({icon:'✅',title:'Livraison validée !',body:`${fmtAmt(pendingValidation.amount)} FCFA libérés à ${pendingValidation.sellerAccount}`,type:'info'});
  toast('✅','Livraison confirmée !',`${fmtAmt(pendingValidation.amount)} FCFA transférés au vendeur`,'success');
  setTimeout(()=>toast('⚖️','Split exécuté','Fonds distribués automatiquement','info'),1200);
...
function cancelEscrow(id) {
  const e = DB.getEscrows().find(x=>x.id===id);
  if (!e) return;
  if (e.status==='locked') { toast('🔒','Annulation impossible','Le vendeur a verrouillé ce paiement','error'); return; }
  DB.patchEscrow(id, {status:'cancelled'});
  updateBalance(e.amount + Math.round(e.amount*0.01));
  DB.addTx({type:'refund',amount:e.amount,description:`Annulé · ${e.productName}`,status:'refunded',statusLabel:'Annulé ↩️'});
  DB.addNotif({icon:'↩️',title:'Paiement annulé',body:`${fmtAmt(e.amount)} FCFA restitués sur votre compte`,type:'warn'});
  toast('↩️','Annulation effectuée',`${fmtAmt(e.amount)} FCFA restitués`,'info');
  nav('escrow');
}

/* PIN */
function pinNext(input, idx) {
  if (input.value && idx<5) document.querySelectorAll('.pin-input')[idx+1]?.focus();
}

function confirmPin() {
  const pins = [...document.querySelectorAll('.pin-input')].map(i=>i.value).join('');
  if (pins.length<6) { toast('⚠️','PIN incomplet','Entrez les 6 chiffres','warn'); return; }
  const pin = currentUser?.pin||'123456';
  if (pins!==pin) { toast('❌','PIN incorrect','Vérifiez votre code','error'); return; }
  closeModal('pinModal');
  ...
  if (pendingEscrow) {
    ...
    const esc2 = DB.addEscrow({...pendingEscrow, status:'pending'});
    DB.addTx({type:'escrow',amount:-pendingEscrow.amount,description:`Séquestre · ${pendingEscrow.name}`,status:'pending',statusLabel:'En attente ⏳',escrowId:esc2.id});
    DB.addNotif({icon:'🔒',title:'Paiement sécurisé lancé',body:`${fmtAmt(pendingEscrow.amount)} FCFA bloqués pour "${pendingEscrow.name}"`,type:'info'});
    toast('🔒','Paiement sécurisé lancé !',`${fmtAmt(pendingEscrow.amount)} FCFA en séquestre`,'success');
    setTimeout(()=>toast('📨','Vendeur notifié',`${pendingEscrow.seller} a été informé`,'info'),1200);
    pendingEscrow = null;
    nav('dashboard');
  }
}
...
function selPm(el, name, type) {
  ...
  if (!fields) return;
  const tpls = {
    phone:`<div class="form-group"><label class="form-label">📱 Numéro de téléphone</label><input type="tel" class="form-input" placeholder="+225 07 00 00 00 00"/></div>`,
    card:`<div class="form-row"><div class="form-group"><label class="form-label">Numéro de carte</label><input type="text" class="form-input" placeholder="0000 0000 0000 0000" maxlength="19"/></div><div class="form-group"><label class="form-label">Titulaire</label><input type="text" class="form-input" placeholder="NOM PRENOM"/></div></div>
          <div class="form-row"><div class="form-group"><label class="form-label">Expiration</label><input type="text" class="form-input" placeholder="MM/AA" maxlength="5"/></div><div class="form-group"><label class="form-label">CVV</label><input type="password" class="form-input" placeholder="•••" maxlength="4"/></div></div>`,
    paypal:`<div class="form-group"><label class="form-label">Email PayPal</label><input type="email" class="form-input" placeholder="vous@email.com"/></div>`,
    crypto:`<div class="form-group"><label class="form-label">Adresse wallet source</label><input type="text" class="form-input" placeholder="1BvBMSEYstWetqTFn5Au4m4GFg7xJaNVN2"/></div>`,
  };
  fields.innerHTML = tpls[type]||'';
}

function setQ(v) { const el=document.getElementById('rchAmount'); if(el) el.value=v; }
...
function doRecharge() {
  ...
  if (!amount||amount<500) { toast('⚠️','Montant invalide','Minimum 500 FCFA','warn'); return; }
  closeModal('rechargeModal');
  updateBalance(amount);
  DB.addTx({type:'credit',amount,description:`Recharge ${selectedPm}`,status:'done',statusLabel:'Réussi ✅'});
  DB.addNotif({icon:'💰',title:'Recharge réussie',body:`+${fmtAmt(amount)} FCFA crédités via ${selectedPm}`,type:'info'});
  toast('💰','Recharge réussie !',`+${fmtAmt(amount)} FCFA ajoutés`,'success');
  selectedPm = '';
  nav('dashboard');
}
...
function initTransfer(type) {
  if (!currentUser) { requireAuth(()=>{}); return; }
  let dest, amount;
  if (type==='coud')  { dest=document.getElementById('trDest')?.value?.trim();  amount=parseInt(document.getElementById('trAmt')?.value||0); }
  if (type==='mobile'){ dest=(document.getElementById('trOp')?.value||'')+':'+document.getElementById('trPhone')?.value?.trim(); amount=parseInt(document.getElementById('trAmtM')?.value||0); }
  if (type==='bank')  { dest=document.getElementById('trIban')?.value?.trim(); amount=parseInt(document.getElementById('trAmtB')?.value||0); }
  if (!dest||!dest.trim().length) { toast('⚠️','Destinataire requis','','warn'); return; }
  if (!amount) { toast('⚠️','Montant requis','','warn'); return; }
  if (amount>currentUser.balance) { toast('❌','Solde insuffisant',`Solde: ${fmtAmt(currentUser.balance)} FCFA`,'error'); return; }
  ...
  document.getElementById('transferInfo').innerHTML = `
    ...
    <div class="kv-row" style="border-bottom:none;"><span class="kv-key">Montant</span><span class="kv-val" style="color:var(--red);">-${fmtAmt(amount)} FCFA</span></div>`;
...
function executeTransfer() {
  ...
  DB.addTx({type:'debit',amount:-pendingTransfer.amount,description:`Transfert → ${pendingTransfer.dest}`,status:'done',statusLabel:'Envoyé ✅'});
  DB.addNotif({icon:'↗️',title:'Transfert effectué',body:`${fmtAmt(pendingTransfer.amount)} FCFA envoyés à ${pendingTransfer.dest}`,type:'info'});
...
function toggleDelivery(btn) {
  ...
  if (card)  { card.style.borderLeftColor=on?'var(--amber)':'var(--border)'; card.style.background=on?'var(--am-light)':'var(--s2)'; }
...
function recalcSplitConfig() {
  ...
  if (pctEl) { pctEl.textContent=total+'%'; pctEl.style.color=total===100?'var(--green)':total>100?'var(--red)':'var(--amber)'; }
  if (cardEl) cardEl.style.background = total===100?'var(--g-light)':total>100?'var(--r-light)':'var(--a-light)';
  const amt   = parseInt(document.getElementById('simAmt')?.value||100000);
  const simEl = document.getElementById('simResults');
  if (!simEl) return;
  simEl.innerHTML = `
    <div class="kv-row"><span class="kv-key" style="display:flex;align-items:center;gap:6px;"><span style="width:8px;height:8px;border-radius:50%;background:var(--green);display:inline-block;"></span>Vendeur (${seller}%)</span><span class="kv-val" style="color:var(--green);">${fmtAmt(Math.round(amt*seller/100))} FCFA</span></div>
    <div class="kv-row"><span class="kv-key" style="display:flex;align-items:center;gap:6px;"><span style="width:8px;height:8px;border-radius:50%;background:var(--accent);display:inline-block;"></span>Plateforme (${platform}%)</span><span class="kv-val" style="color:var(--accent);">${fmtAmt(Math.round(amt*platform/100))} FCFA</span></div>
    ${delivOn?`<div class="kv-row"><span class="kv-key" style="display:flex;align-items:center;gap:6px;"><span style="width:8px;height:8px;border-radius:50%;background:var(--amber);display:inline-block;"></span>Livreur (${delivery}%)</span><span class="kv-val" style="color:var(--amber);">${fmtAmt(Math.round(amt*delivery/100))} FCFA</span></div>`:''}
    <div class="kv-row" style="border-bottom:none;border-top:1px solid var(--border);padding-top:10px;"><span class="kv-key" style="font-weight:700;">Total</span><span class="kv-val" style="color:${total===100?'var(--green)':total>100?'var(--red)':'var(--amber)'};">${total}%${total===100?' ✅':total>100?' ⚠️ Dépasse 100%':' ⚠️ Doit être 100%'}</span></div>`;
}
...
function saveSplitConfig() {
  ...
  const delivery = delivOn?parseInt(document.getElementById('sp-delivery')?.value||0):0;
  const total    = seller+platform+delivery;
  if (total!==100) { toast('⚠️','Total ≠ 100%',`Total actuel: ${total}%. Ajustez les pourcentages.`,'warn'); return; }
  const cfg = DB.getDevCfg();
  cfg.split_config = {seller, platform, delivery, delivery_enabled:delivOn};
  DB.setDevCfg(cfg);
  const rules = DB.getSplitRules();
  rules.unshift({id:'split-'+Date.now(),name:`Config ${fmtDateShort(new Date())}`,beneficiaries:[{name:'Vendeur',pct:seller},{name:'Plateforme',pct:platform}].concat(delivOn?[{name:'Livreur',pct:delivery}]:[]),createdAt:new Date().toISOString()});
  DB.setSplitRules(rules.slice(0,10));
  toast('💾','Configuration sauvegardée',`Vendeur ${seller}% · Plateforme ${platform}%${delivOn?' · Livreur '+delivery+'%':''}`, 'success');
}

/* API KEYS */
function genKey(env) {
  if (!currentUser) { requireAuth(()=>{}); return; }
  const name = prompt('Nom de la clé (ex: "Site principal") :');
  if (name===null) return;
...
function copyApiKey() { copyText(generatedApiKey); closeModal('apiKeyModal'); }
function revealSk(id,sk) { const el=document.getElementById('sk-'+id); if(el) el.textContent=el.textContent.includes('•')?sk:sk.slice(0,12)+'••••••••••••••••••••'; }
function deleteApiKey(id) {
  if (!confirm('Supprimer cette clé ? Irréversible.')) return;
...
function testPmConfig(id) {
  toast('🧪','Test en cours...',`Vérification ${id.replace(/_/g,' ')}`,'info');
  setTimeout(()=>toast('✅','Connexion établie',`${id.replace(/_/g,' ')} correctement configuré`,'success'),2000);
}

function saveDevConfig() {
  const cfg = DB.getDevCfg();
  ['orange_money','mtn_money','wave','moov','mastercard','visa','paypal','stripe','crypto'].forEach(id=>{
    const tog = document.getElementById('toggle-'+id);
    if (!tog) return;
    cfg.payment_methods[id] = cfg.payment_methods[id]||{};
    cfg.payment_methods[id].enabled = tog.classList.contains('on');
    document.querySelectorAll(`[id^="pm-${id}-"]`).forEach(f=>{
      cfg.payment_methods[id][f.id.replace('pm-'+id+'-','')] = f.value;
    });
  });
  DB.setDevCfg(cfg);
  toast('💾','Configuration sauvegardée','Tous les moyens de paiement mis à jour','success');
}
...
function renderWebhookList() {
  ...
  if (!el) return;
  if (!cfg.webhooks?.length) { el.innerHTML='<p style="font-size:13px;color:var(--muted);padding:10px 0;">Aucun endpoint configuré</p>'; return; }
  el.innerHTML = cfg.webhooks.map(w=>`
    <div style="display:flex;align-items:center;gap:10px;padding:10px;background:var(--s2);border-radius:var(--rs);margin-bottom:8px;">
      ...
      <span class="chip chip-blue" style="font-size:10px;">${w.events?.length||0} events</span>
      <button class="btn btn-ghost btn-sm" onclick="toast('📨','Test envoyé','Payload test envoyé','info')">Tester</button>
    </div>`).join('');
}
...
function saveWebhook() {
  ...
  const events = [...document.querySelectorAll('#webhookList ~ * input[type="checkbox"]:checked, .overlay input[type="checkbox"]:checked')].map(i=>i.value).filter(Boolean);
  const cfg = DB.getDevCfg();
  cfg.webhooks = cfg.webhooks||[];
  cfg.webhooks.push({url, events:['escrow.created','escrow.validated','escrow.refunded'], active:true, createdAt:new Date().toISOString()});
  DB.setDevCfg(cfg);
...
function changePin() {
  ...
  if (np.length!==6||!/^\d+$/.test(np)) { toast('⚠️','PIN invalide','6 chiffres requis','warn'); return; }
  if (np!==cf) { toast('⚠️','Confirmation incorrecte','','warn'); return; }
  currentUser.pin = np;
  const users = DB.getUsers().map(u=>u.id===currentUser.id?currentUser:u);
  DB.setUsers(users);
  DB.setSession(currentUser);
...
function showTxDetail(txId) {
  ...
  const typeLabel = {escrow:'🔒 Séquestre',credit:'💰 Crédit',debit:'↗️ Débit',split:'⚖️ Split',refund:'↩️ Remboursement'}[t.type]||t.type;
  const smMap = {pending:'chip-amber',done:'chip-green',failed:'chip-red',refunded:'chip-blue'};
  showDetailModal('📋 Détail transaction', `
    ...
    <div style="background:var(--s2);border-radius:var(--rs);padding:14px;">
      <div class="kv-row"><span class="kv-key">ID</span><code style="font-size:11px;color:var(--accent);">${t.id}</code></div>
    ...
    <button class="btn btn-ghost btn-full" style="margin-top:12px;" onclick="copyText('${t.id}')">📋 Copier l'ID</button>`);
}
...
function showNotifDetail(notifId) {
  ...
  showDetailModal(`${n.icon||'🔔'} Notification`, `
    ...
    <p style="font-size:14px;color:var(--t2);line-height:1.7;margin-bottom:16px;">${esc(n.body)}</p>
    <div style="background:var(--s2);border-radius:var(--rs);padding:12px;">
      ...
      <div class="kv-row" style="border-bottom:none;"><span class="kv-key">Date</span><span class="kv-val">${fmtDateShort(new Date(n.ts))}</span></div>
    </div>`);
}
...
function showDetailModal(title, bodyHtml) {
  ...
  if (!ov) {
    ...
    ov.className='overlay'; ov.id='detailModal';
    ov.innerHTML=`<div class="modal"><button class="modal-close" onclick="closeModal('detailModal')">✕</button><div id="dtTitle" class="modal-title" style="margin-bottom:14px;font-size:17px;"></div><div id="dtBody"></div></div>`;
...
function toggleEp(header) {
  const body=header.nextElementSibling, arrow=header.querySelector('.ep-arrow');
...
/* ─────────────────────────────────────
   UTILITAIRES
───────────────────────────────────── */
function fmtAmt(n)      { return Math.round(n||0).toLocaleString('fr-FR'); }
function fmtDate(d)     { return d.toLocaleDateString('fr-FR',{day:'numeric',month:'long',year:'numeric'}); }
function fmtDateShort(d){ return d.toLocaleDateString('fr-FR',{day:'2-digit',month:'2-digit',year:'numeric'})+' '+d.toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'}); }
...
function copyText(text) {
  ...
  navigator.clipboard?.writeText(clean).then(()=>toast('📋','Copié !','','success')).catch(()=>toast('📋','Copié','','success'));
}
...
function exportCSV() {
  ...
  const rows=txs.map(t=>`${t.id},"${t.description}",${t.type},${t.amount},${t.status},${t.ts}`).join('\n');
  const a=document.createElement('a');
  a.href=URL.createObjectURL(new Blob([header+rows],{type:'text/csv'}));
  a.download='cloudpay_transactions.csv'; a.click();
